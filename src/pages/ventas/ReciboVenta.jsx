...
function ReciboVenta() {
  ...
  const subtotalBruto = nota.detallesVenta.reduce(
    (acc, d) => acc + d.cantidad * parseFloat(d.precioVenta),
    0
  );
  const descuento = Number(nota.descuento) || 0;
  ...
  return (
    <>
      ...
      <div className="recibo-acciones">
        ...
        <Link to={`/ventas/notas/${id}`} className="recibo-btn-volver" style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <FaArrowLeft /> Volver
        </Link>
      </div>
      ...
      <div className="recibo-wrapper">
        ...
        <div className="recibo-header">
          ...
          <div style={{ textAlign: "right" }}>
            <h2 className="recibo-titulo">RECIBO DE VENTA</h2>
        ...
        <div className="recibo-partes">
          <div className="recibo-parte">
            <p className="recibo-parte-titulo">CLIENTE</p>
            <p className="recibo-parte-nombre">
              {cliente.nombre} {cliente.apellidoPaterno}{cliente.apellidoMaterno ? ` ${cliente.apellidoMaterno}` : ""}
            </p>
            <p className="recibo-parte-detalle">Correo: {cliente.correo}</p>
            <p className="recibo-parte-detalle">Tel: {cliente.telefono}</p>
            <p className="recibo-parte-detalle">{cliente.direccion}</p>
          </div>
          <div className="recibo-parte">
        ...
        <table className="recibo-tabla">
            ...
            {nota.detallesVenta.map((d) => {
              ...
              return (
                <tr key={d.detalleVentaId}>
                  ...
                  <td style={{ textAlign: "right" }}>Bs. {Number(d.precioVenta).toFixed(2)}</td>
          ...
          <tfoot>
            {descuento > 0 && (
              <>
                <tr>
                  <td colSpan={5} style={{ textAlign: "right", paddingTop: 10, color: "#6b7280" }}>Subtotal</td>
            ...
            <tr className="recibo-total-row">
              <td colSpan={5} style={{ textAlign: "right" }}>TOTAL VENTA</td>
              <td style={{ textAlign: "right" }}>Bs. {Number(nota.montoTotal).toFixed(2)}</td>
            </tr>
          </tfoot>
        </table>
        ...
        <p className="recibo-seccion-titulo">INFORMACIÓN DE COBRO</p>

        {!nota.esCredito && nota.movimientoCaja && (
          <div className="recibo-pago-contado">
            <p>Modalidad: <strong>Cobro al contado</strong></p>
            <p>Fecha de cobro: <strong>{new Date(nota.movimientoCaja.fecha).toLocaleDateString("es-BO")}</strong></p>
            <p>Monto cobrado: <strong>Bs. {Number(nota.movimientoCaja.monto).toFixed(2)}</strong></p>
          ...
          <div className="recibo-pago-credito">
            <div className="recibo-pago-credito-grid">
              <div>
                ...
                <p style={{ fontWeight: 700 }}>{ESTADO_LABEL[nota.cuentaPorCobrar.estado] ?? nota.cuentaPorCobrar.estado}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p style={{ fontWeight: 700 }}>
                  {nota.cuentaPorCobrar.fechaVencimiento
                    ? new Date(nota.cuentaPorCobrar.fechaVencimiento).toLocaleDateString("es-BO")
                    : "—"}
                </p>
              </div>
            </div>
            ...
            {nota.cuentaPorCobrar.movimientosCaja.length > 0 && (
              <>
                <p className="recibo-parte-titulo" style={{ marginTop: 12 }}>HISTORIAL DE COBROS</p>
        ...
        <div className="recibo-firmas">
          <div className="recibo-firma">
            ...
            <p>Firma Responsable de Venta</p>
          </div>
          <div className="recibo-firma">
            <div className="recibo-firma-linea" />
            <p>Firma Cliente</p>
